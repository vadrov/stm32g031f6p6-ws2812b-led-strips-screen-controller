...
#define SD_INI_SPEED		7 		//скорость SPI при инициализации карты:
/*									0 - FCLK/2,
									1 - FCLK/4,
									2 - FCLK/8,
									3 - FCLK/16,
...
#define SD_DMA_CHANNEL_RX	LL_DMA_CHANNEL_2	//номер канала DMA для входящих данных
#define SD_DMA_CNTR_TX		DMA1				//контроллер DMA для исходящих данных
#define SD_DMA_CHANNEL_TX	LL_DMA_CHANNEL_3	//номер канала DMA для исходящих данных
#endif
...
//SPI DMA прием/передача полный дуплекс мастер без внешних прерываний по событиям
//(обработка флагов внутри)
static void SPI_DMA_TransmitReceive(SPI_TransferType type, const BYTE* buff, uint16_t buff_size)
{
	uint8_t tmp = 0xFF;
	//определяем базовые адреса каналов DMA
	DMA_Channel_TypeDef *dma_RX = ((DMA_Channel_TypeDef *)((uint32_t)((uint32_t)SD_DMA_CNTR_RX + CHANNEL_OFFSET_TAB[SD_DMA_CHANNEL_RX])));
	DMA_Channel_TypeDef *dma_TX = ((DMA_Channel_TypeDef *)((uint32_t)((uint32_t)SD_DMA_CNTR_TX + CHANNEL_OFFSET_TAB[SD_DMA_CHANNEL_TX])));
	//сбрасываем флаги прерываний потока rx
	SD_DMA_CNTR_RX->IFCR |= DMA_IFCR_CGIF1 << (SD_DMA_CHANNEL_RX * 4);
	//сбрасываем флаги прерываний потока tx
	SD_DMA_CNTR_TX->IFCR |= DMA_IFCR_CGIF1 << (SD_DMA_CHANNEL_TX * 4);
	//разрешаем spi отправлять запросы к DMA
	SPI_SD->CR2 |= (SPI_CR2_RXDMAEN | SPI_CR2_TXDMAEN);
	//Количество данных для приема/передачи четное, т.к. ширина кадра spi совпадает с шириной кадра памяти
	SPI_SD->CR2 &= ~(SPI_CR2_LDMATX | SPI_CR2_LDMARX);
	//запрещаем прерывания по всем событиям потока приема rx
	dma_RX->CCR &= ~(DMA_CCR_HTIE | DMA_CCR_TEIE | DMA_CCR_TCIE);
	//запрещаем прерывания по всем событиям потока передачи tx
	dma_TX->CCR &= ~(DMA_CCR_HTIE | DMA_CCR_TEIE | DMA_CCR_TCIE);
	//включаем SPI
	SPI_SD->CR1 |= SPI_CR1_SPE;
	//настраиваем каналы DMA для приема и передачи данных с карты/на карту
	if(type == SPI_Transfer_Receive) //прием
	{
		dma_RX->CCR &= ~DMA_CCR_PINC; //инкремент адреса периферии приемника отключен
		dma_RX->CCR |= DMA_CCR_MINC;  //инкремент адреса памяти приемника включен
		dma_TX->CCR &= ~(DMA_CCR_PINC | DMA_CCR_MINC); //инкремент адресов периферии и памяти источника отключен
		dma_TX->CMAR = (uint32_t)&tmp; //адрес в памяти с данными для исходящего канала
		dma_RX->CMAR = (uint32_t)buff; //адрес в памяти для данных входящего канала
	}
	else if (type == SPI_Transfer_Transmit) //передача
	{
		dma_RX->CCR &= ~(DMA_CCR_PINC | DMA_CCR_MINC); //инкремент адресов периферии и памяти приемника отключен
		dma_TX->CCR &= ~DMA_CCR_PINC; //инкремент адреса периферии источника отключен
		dma_TX->CCR |= DMA_CCR_MINC;  //инкремент адреса памяти источника включен
		dma_TX->CMAR = (uint32_t)buff; //адрес в памяти с данными для исходящего канала
		dma_RX->CMAR = (uint32_t)&tmp; //адрес в памяти для данных входящего канала
	}
	dma_RX->CPAR = dma_TX->CPAR = (uint32_t)(&SPI_SD->DR); //приемник/источник периферия - адрес регистра DR spi
	dma_RX->CNDTR = dma_TX->CNDTR = (uint32_t)buff_size; //размер принимаемых/передаваемых данных
	dma_RX->CCR |= (DMA_CCR_EN); //включение канала приема
	dma_TX->CCR |= (DMA_CCR_EN); //включение канала передачи
	//Ждем, пока не установятся флаги окончания приема/передачи.
	while (!( (SD_DMA_CNTR_RX->ISR & (DMA_ISR_TCIF1 << (SD_DMA_CHANNEL_RX * 4))) &&
			  (SD_DMA_CNTR_TX->ISR & (DMA_ISR_TCIF1 << (SD_DMA_CHANNEL_TX * 4)))    ) ) ;
	//выключаем каналы DMA
	dma_TX->CCR &= ~DMA_CCR_EN;
	dma_RX->CCR &= ~DMA_CCR_EN;
	while ((dma_TX->CCR & DMA_CCR_EN) || (dma_RX->CCR & DMA_CCR_EN)) ; //ждем отключения каналов DMA
