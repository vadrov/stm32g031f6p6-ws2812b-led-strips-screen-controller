 ...
 *
 *  Версия: 1.4G0 (CMSIS) для STM32G0
 *
...
#define LCD_CS_HI		if (lcd->spi_data.cs_port) { lcd->spi_data.cs_port->BSRR = (uint32_t)lcd->spi_data.cs_pin; }
#define LCD_DC_LOW		{lcd->spi_data.dc_port->BSRR = (uint32_t)lcd->spi_data.dc_pin << 16U; }
#define LCD_DC_HI		{lcd->spi_data.dc_port->BSRR = (uint32_t)lcd->spi_data.dc_pin; }
...
void Display_TC_Callback(DMA_TypeDef *dma_x, uint32_t channel)
{
	...
	while (lcd) {
		...
		if (dma_ct == dma_x && channel_ct == channel) {
			...
			DMA_Channel_TypeDef *dma_TX = ((DMA_Channel_TypeDef *)((uint32_t)((uint32_t)dma_x + 8U + 20U * (channel-1))));
			//выключаем поток DMA
			dma_TX->CCR &= ~DMA_CCR_EN;
			while (dma_TX->CCR & DMA_CCR_EN) ; //ждем отключения потока
...
#else
inline void Display_TC_Callback(DMA_TypeDef *dma_x, uint32_t channel)
{
	if (!LCD) return;
	...
	if (LCD->size_mem) {
		if (LCD->size_mem > 65535) {
			DMA1_Channel1->CNDTR = 65535;
			LCD->size_mem -= 65535;
		}
		else {
			DMA1_Channel1->CNDTR = LCD->size_mem;
			LCD->size_mem = 0;
		}
		DMA1_Channel1->CCR |= DMA_CCR_EN;
		return;
	...
	if (!LCD->cs_control) {
		if (LCD->spi_data.cs_port) {
			LCD->spi_data.cs_port->BSRR = (uint32_t)LCD->spi_data.cs_pin;
		}
	}
...
void LCD_HardWareReset (LCD_Handler* lcd)
{
	if (lcd->spi_data.reset_port) {
		...
		LCD_Delay(25);
		lcd->spi_data.reset_port->BSRR = (uint32_t)lcd->spi_data.reset_pin;
		LCD_Delay(25);
	}
}
...
void LCD_ReadImage(LCD_Handler* lcd, uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t *data)
{
	...
	SPI_TypeDef *spi = lcd->spi_data.spi;
	uint32_t spi_cr1 = spi->CR1; //Запоминаем параметры spi
	uint32_t spi_cr2 = spi->CR2;
	...
	spi->CR1 = spi_cr1; //Восстанавливаем прежние параметры spi
	spi->CR2 = spi_cr2;
					   ...
					   )
{
	...
	LCD_DMA_TypeDef *dma = 0;
	lcd->data_bus = data_bus;
	//инициализация данных подключения
	lcd->spi_data = *((LCD_SPI_Connected_data*)connection_data);
	dma = &lcd->spi_data.dma_tx;
	//настройка DMA
	if (dma->dma) {
		DMA_Channel_TypeDef *dma_x = ((DMA_Channel_TypeDef *)((uint32_t)((uint32_t)dma->dma + 8U + 20U * (dma->channel - 1))));
		dma_x->CCR &= ~DMA_CCR_EN; //отключаем канал DMA
		while(dma_x->CCR & DMA_CCR_EN) ; //ждем отключения канала
		dma_x->CCR &= ~(DMA_CCR_MSIZE | DMA_CCR_PSIZE); //ширина данных памяти и периферии 8 бит
		if (lcd->data_bus == LCD_DATA_16BIT_BUS) { //ширина данных памяти и периферии 16 бит
			dma_x->CCR |= DMA_CCR_MSIZE_0 | DMA_CCR_PSIZE_0;
		}
		//запрещаем прерывания по ошибке и передачи половины данных
		dma_x->CCR &= ~(DMA_CCR_HTIE | DMA_CCR_TEIE);
		//разрешаем прерывание по окончанию передачи
		dma_x->CCR |= DMA_CCR_TCIE;
		dma_x->CCR &= ~DMA_CCR_PINC; //инкремент адреса периферии отключен
		dma_x->CCR |= DMA_CCR_MINC;  //инкремент адреса памяти включен
	}
...
void LCD_WriteData(LCD_Handler *lcd, uint16_t *data, uint32_t len)
{
	...
	if (lcd->data_bus == LCD_DATA_16BIT_BUS) {
		spi->CR2 |= 15UL << SPI_CR2_DS_Pos; //ширины кадра spi 16 бит
	}
	else {
		spi->CR2 |= 7UL << SPI_CR2_DS_Pos; //ширины кадра spi 8 бит
	}
...
void LCD_WriteDataDMA(LCD_Handler *lcd, uint16_t *data, uint32_t len)
{
	if (lcd->spi_data.dma_tx.dma) {
		...
		if (len <= 65535) {
			dma_TX->CNDTR = (uint32_t)len; //размер передаваемых данных
			lcd->size_mem = 0;
		}
		else {
			dma_TX->CNDTR = 65535;
		...
		dma_TX->CCR |= DMA_CCR_EN; //включение канала передачи (старт DMA передачи)
		return;
	}
