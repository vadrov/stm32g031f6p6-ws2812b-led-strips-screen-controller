...
uint16_t KEYB_old_keys = 0;						//Предыдущее состояние клавиш (установленный бит - нажата соответствующая клавиша).
												//Размерность переменной в 16 бит позволяет хранить информацию о состоянии 16 кнопок
...
#endif
uint16_t KEYB_key_buff[KEYB_SIZE_BUFFER] = {0}; //Буфер хранения состояний кнопок.
...
void KEYB_Input_Keys(void)
{
	...
	uint16_t keys = 0;//переменная для хранения состояний кнопок
	int i = 0;
